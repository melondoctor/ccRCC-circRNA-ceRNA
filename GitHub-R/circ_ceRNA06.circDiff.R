...
write.table(allDiffOut,file="circ.all.xls",sep="\t",quote=F,col.names=F)

#输出矫正后的表达量
outData=rbind(id=paste0(colnames(data),"_",Type),data)
write.table(outData,file="circ.normalize.txt",sep="\t",quote=F,col.names=F)

#输出差异结果
diffSig=allDiff[with(allDiff, (abs(logFC)>logFCfilter & adj.P.Val < adj.P.Val.Filter )), ]
diffSigOut=rbind(id=colnames(diffSig),diffSig)
write.table(diffSigOut,file="circ.diff.xls",sep="\t",quote=F,col.names=F)
write.table(diffSigOut,file="circ.diff.txt",sep="\t",quote=F,col.names=F)
...
pdf(file="circ.heatmap.pdf",height=7,width=8)
pheatmap(hmExp, 
         annotation=Type, 
