...
write.table(allDiffOut,file="mirna.all.xls",sep="\t",quote=F,col.names=F)

#输出矫正后的表达量
outData=rbind(id=paste0(colnames(data),"_",Type),data)
write.table(outData,file="mirna.normalize.txt",sep="\t",quote=F,col.names=F)

#输出差异结果
diffSig=allDiff[with(allDiff, (abs(logFC)>logFCfilter & adj.P.Val < adj.P.Val.Filter )), ]
diffSigOut=rbind(id=colnames(diffSig),diffSig)
write.table(diffSigOut,file="mirna.diff.xls",sep="\t",quote=F,col.names=F)
write.table(diffSigOut,file="mirna.diff.txt",sep="\t",quote=F,col.names=F)
...
pdf(file="mirna.heatmap.pdf",height=8,width=9)
pheatmap(hmExp, 
         annotation=Type, 
